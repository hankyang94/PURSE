...
for objidx = 1:n_objs
    ...
    obj_R_bounds = zeros(n_samples,1);
    obj_t_bounds = zeros(n_samples,1);
    ...
    for idx = 1:n_samples
        ...
        R_query = squeeze(obj_R_samples{idx}(pose_id,:,:));
        t_query = squeeze(obj_t_samples{idx}(pose_id,:));
        t_query = t_query(:);

        [R_bd, t_bd] = compute_bound_one_instance(A_c,B_c,R_query,t_query);
        obj_R_bounds(idx) = R_bd;
        obj_t_bounds(idx) = t_bd;
    end
